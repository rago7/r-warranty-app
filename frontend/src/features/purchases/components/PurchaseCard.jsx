...
export default function PurchaseCard({ purchase }) {
    ...
    return (
        <Link to={`/purchases/${purchase.id}`} className="card overflow-hidden transition-colors hover:bg-[rgb(var(--surface-hover))]">
            <div className="bg-[#F0FDFA] px-[1.25rem] py-2 border-b border-[rgb(var(--border))]">
                <div className="flex items-center gap-3">
                    <h3 className="text-base font-semibold">
                        {purchase.merchant || purchase.title || purchase.product_name}
                    </h3>
            ...
            <div className="card-padded grid grid-cols-[1fr_auto] gap-2">
                <div>
                    <div className="text-sm text-[rgb(var(--muted-fg))]">
                        <span><span className="font-medium">Purchased At</span>: {purchase.merchant}</span>
                        <span className="mx-2">•</span>
                        <span><span className="font-medium">Date</span>: {formatDate(purchase.purchase_datetime || purchase.purchase_date, { timeZone: prefs.timezone, locale: prefs.locale })}</span>
                        {purchase.purchase_category ? <><span className="mx-2">•</span><span><span className="font-medium">Category</span>: <span className="capitalize">{purchase.purchase_category}</span></span></> : null}
                    </div>
                ...
                <div className="text-right">
                    ...
                    <div className="text-lg font-semibold">{formatMoney(purchase.total_amount, prefs.currency || purchase.currency, prefs.locale)}</div>
                </div>
            </div>
        </Link>
    )
